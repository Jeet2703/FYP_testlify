...
const JobModal = ({ job, isOpen, onClose, onApplicationSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { skillMatch, experienceMatch } = response.data;

      if (skillMatch < 50 || !experienceMatch) {
        toast.error(
          `Your skills or experience do not meet the job requirements (${skillMatch}% match)`
        );
        return;
      }
